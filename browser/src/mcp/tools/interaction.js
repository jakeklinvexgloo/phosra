...
function registerInteractionTools(server, getBrowser) {
    ...
    server.tool('browser_type', 'Type text into a focused element or an element specified by selector', {
        text: zod_1.z.string().describe('Text to type'),
        selector: zod_1.z.string().optional().describe('CSS selector to focus before typing'),
        clear: zod_1.z.boolean().optional().describe('Clear the field before typing'),
        ...
        try {
            ...
            page.close();
            return { content: [{ type: 'text', text: `Typed "${text.length > 50 ? text.slice(0, 50) + '...' : text}"` }] };
        }
        catch (err) {
            page.close();
            return { content: [{ type: 'text', text: `Error: ${err.message}` }], isError: true };
    ...
    server.tool('browser_screenshot', 'Take a screenshot of the current page', {
        fullPage: zod_1.z.boolean().optional().describe('Capture the full scrollable page'),
        selector: zod_1.z.string().optional().describe('CSS selector to screenshot a specific element'),
        ...
        try {
            ...
            const screenshot = await page.send('Page.captureScreenshot', params);
            page.close();
            return {
                content: [{
                        type: 'image',
    ...
    server.tool('browser_scroll', 'Scroll the page up or down', {
        ...
        amount: zod_1.z.number().optional().describe('Scroll amount in pixels (default 500)'),
    ...
    server.tool('browser_hover', 'Hover over an element by CSS selector', {
        selector: zod_1.z.string().describe('CSS selector of the element to hover'),
    }, async ({ selector }) => {
        const page = await getPage();
        try {
            const result = await page.send('Runtime.evaluate', {
    ...
    server.tool('browser_press_key', 'Press a keyboard key or combination', {
        key: zod_1.z.string().describe('Key or key combination (e.g. "Enter", "Control+A", "Tab")'),
    }, async ({ key }) => {
        const page = await getPage();
        try {
