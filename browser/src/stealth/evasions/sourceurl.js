...
function sourceurl() {
    try {
        ...
        function containsSuspiciousSource(str) {
        ...
        ErrorConstructor.prepareStackTrace = function (error, structuredStackTrace) {
            // Filter out frames with suspicious file names
            const filtered = structuredStackTrace.filter((frame) => {
                const fileName = frame.getFileName();
                if (fileName && containsSuspiciousSource(fileName))
                    return false;
                return true;
            });
