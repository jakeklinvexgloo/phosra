...
export default function canvasNoise(): void {
  try {
    ...
    function addNoise(canvas: HTMLCanvasElement): void {
      try {
        ...
        for (let i = 0; i < NOISE_PIXEL_COUNT && i < totalPixels; i++) {
          ...
          const idx =
            (Math.floor(Math.random() * totalPixels) * 4) % data.length;
          // Modify RGB channels by a tiny random offset; leave alpha alone
          for (let c = 0; c < 3; c++) {
            const offset =
              Math.floor(Math.random() * (NOISE_INTENSITY * 2 + 1)) -
              NOISE_INTENSITY;
    ...
    HTMLCanvasElement.prototype.toBlob = function (
      ...args: Parameters<typeof origToBlob>
    ): void {
      addNoise(this);
