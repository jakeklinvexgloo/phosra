...
exports.default = pluginsMimetypes;
/**
...
function pluginsMimetypes() {
    try {
        ...
        function makePlugin(name, description, filename, 
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        mimeTypes) {
            ...
            p.item = (index) => mimeTypes[index] || null;
            p.namedItem = (name) => mimeTypes.find(
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            (mt) => mt.type === name) || null;
            return p;
        }
        // ---- build fake data ----
        const pdfPlugin = makePlugin('Chrome PDF Plugin', 'Portable Document Format', 'internal-pdf-viewer', []);
        const pdfMime = makeMimeType('application/pdf', 'pdf', 'Portable Document Format', pdfPlugin);
        const pdfXMime = makeMimeType('application/x-google-chrome-pdf', 'pdf', 'Portable Document Format', pdfPlugin);
        // Back-reference
        ...
        const pdfViewerPlugin = makePlugin('Chrome PDF Viewer', 'Portable Document Format', 'internal-pdf-viewer', [pdfMime]);
        const naclPlugin = makePlugin('Native Client', '', 'internal-nacl-plugin', []);
        ...
        fakePluginArray.item = (index) => allPlugins[index] || null;
        fakePluginArray.namedItem = (name) => 
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        allPlugins.find((p) => p.name === name) || null;
        ...
        fakeMimeTypeArray.item = (index) => allMimeTypes[index] || null;
        fakeMimeTypeArray.namedItem = (name) => 
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        allMimeTypes.find((mt) => mt.type === name) || null;
