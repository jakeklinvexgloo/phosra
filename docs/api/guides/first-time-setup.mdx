...
```bash
curl -X POST https://phosra-api-production.up.railway.app/api/v1/auth/register \
...
```bash
curl -X POST https://phosra-api-production.up.railway.app/api/v1/webhooks \
  -H "Authorization: Bearer $PHOSRA_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
    "family_id": "FAMILY_UUID",
...
```bash
curl -X POST https://phosra-api-production.up.railway.app/api/v1/children/CHILD_UUID/enforce \
  -H "Authorization: Bearer $PHOSRA_API_KEY"
```
