...
```bash
curl https://phosra-api-production.up.railway.app/api/v1/platforms \
...
```bash
curl "https://phosra-api-production.up.railway.app/api/v1/platforms/by-category?category=dns" \
  -H "Authorization: Bearer $PHOSRA_API_KEY"
```
...
curl "https://phosra-api-production.up.railway.app/api/v1/platforms/netflix/oauth/authorize?redirect_uri=https://yourapp.com/callback" \
  -H "Authorization: Bearer $PHOSRA_API_KEY"

# 2. User authorizes in browser, redirected to your callback with ?code=...

# 3. Exchange the code
curl "https://phosra-api-production.up.railway.app/api/v1/platforms/netflix/oauth/callback?code=AUTH_CODE&redirect_uri=https://yourapp.com/callback" \
  -H "Authorization: Bearer $PHOSRA_API_KEY"
```
...
```bash
curl -X POST https://phosra-api-production.up.railway.app/api/v1/compliance/LINK_UUID/verify \
  -H "Authorization: Bearer $PHOSRA_API_KEY"
```
...
```bash
curl https://phosra-api-production.up.railway.app/api/v1/enforcement/jobs/JOB_UUID/results \
  -H "Authorization: Bearer $PHOSRA_API_KEY"
```
