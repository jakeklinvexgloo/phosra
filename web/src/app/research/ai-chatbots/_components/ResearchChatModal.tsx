...
export function ResearchChatModal({ open, onClose, initialPrompt }: ResearchChatModalProps) {
  ...
  const handleClose = useCallback(() => {
    onClose()
    // Increment key to force a fresh session on next open
    setSessionKey((k) => k + 1)
  }, [onClose])
...
// ── Inner component: remounts on each session for clean state ────────
...
function ResearchChatModalInner({ initialPrompt, onClose, sessionId }: InnerProps) {
  ...
  useEffect(() => {
    if (initialPrompt && !hasSentRef.current) {
  ...
  // ResizeObserver: scroll when content height changes
  useEffect(() => {
      ...
      <div ref={scrollRef} className="flex-1 overflow-y-auto overscroll-y-contain px-5 py-4 space-y-4 min-h-0">
        {messages.length === 0 && !isLoading ? (
          ...
          messages.map((msg) => {
            ...
            // Assistant message: strip follow-ups from rendered text
            const rawText = textOf(msg)
            const { text: cleanText } = extractFollowUps(rawText)

            return (
              <div key={msg.id} className="w-full text-white/80 text-sm prose prose-sm prose-invert max-w-none [&_p]:my-1.5 [&_ul]:my-1.5 [&_ol]:my-1.5 [&_li]:my-0.5 [&_h2]:text-sm [&_h2]:text-white/90 [&_h3]:text-sm [&_h3]:text-white/90 [&_a]:text-brand-green [&_strong]:text-white/90 [&_code]:text-brand-green/80 [&_code]:bg-white/[0.06] [&_code]:px-1 [&_code]:rounded">
          ...
          <div className="flex flex-wrap gap-2 pt-1">
            {followUps.map((q) => (
              <button
                key={q}
                onClick={() => handleSuggestion(q)}
                className="text-xs px-3 py-1.5 rounded-full border border-brand-green/20 bg-brand-green/[0.06] hover:bg-brand-green/[0.12] text-brand-green/80 hover:text-brand-green transition-colors"
              >
                {q}
              </button>
            ))}
