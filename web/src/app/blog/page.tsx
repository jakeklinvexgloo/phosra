...
import { BLOG_POSTS, BLOG_CATEGORY_CONFIG, getFeaturedPost, type BlogCategory } from "@/lib/blog"
...
export default function BlogPage() {
  ...
  const filtered = filter === "all"
    ? BLOG_POSTS.filter((e) => e.slug !== featured?.slug)
    : BLOG_POSTS.filter((e) => e.category === filter && e.slug !== featured?.slug)
    ...
    <div>
        ...
        <div className="relative max-w-5xl mx-auto px-4 sm:px-8 py-20 sm:py-28">
          <AnimatedSection>
            ...
            <p className="text-base sm:text-lg text-white/50 mt-6 max-w-2xl leading-relaxed">
              Technical deep-dives, product launches, regulatory analysis, and company news.
            </p>
          </AnimatedSection>
        </div>
      </section>
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-8 py-16 sm:py-20">
            ...
            <Link href={`/blog/${featured.slug}`} className="group block">
              <div className="plaid-card p-8 sm:p-10 transition-shadow hover:shadow-lg">
                <div className="flex items-center gap-3 mb-4">
                  ...
                  <span className={`text-[10px] font-bold uppercase tracking-wider px-2 py-0.5 rounded ${BLOG_CATEGORY_CONFIG[featured.category].bg} ${BLOG_CATEGORY_CONFIG[featured.category].text}`}>
                    {BLOG_CATEGORY_CONFIG[featured.category].label}
                  </span>
        ...
        <div className="flex items-center gap-2 mb-10 overflow-x-auto pb-1">
          {(["all", "engineering", "product", "company", "regulatory", "milestone"] as const).map((cat) => {
            const label = cat === "all" ? "All" : BLOG_CATEGORY_CONFIG[cat].label
            return (
              <button
                key={cat}
