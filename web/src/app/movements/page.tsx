...
function MovementHubCard({ movement }: { movement: MovementEntry }) {
  const statusMeta = STATUS_META[movement.status]

  return (
    <Link href={`/movements/${movement.slug}`} className="group block h-full">
      <div className="relative h-full rounded-xl border border-border bg-card p-6 hover:border-brand-green/30 hover:shadow-[0_0_24px_-8px_rgba(0,212,126,0.15)] transition-all duration-300">
        ...
        <div
          ...
          style={{ background: `linear-gradient(90deg, ${movement.accentColor}60, ${movement.accentColor}20)` }}
        />

        <div className="flex items-start justify-between mb-4 mt-1">
          <div className="flex items-center gap-3">
            <MovementIcon movement={movement} size="lg" />
            <div>
              <h3 className="text-lg font-semibold text-foreground group-hover:text-brand-green transition-colors">
                {movement.name}
              </h3>
              <p className="text-xs text-muted-foreground">{movement.organization}</p>
            </div>
          </div>
          <span
        ...
        <div className="space-y-2 mb-5">
          {movement.rules.slice(0, 3).map((rule) => (
            <div
              key={rule.category}
          ...
          {movement.rules.length > 3 && (
            <p className="text-xs text-muted-foreground/50 pl-5">
              +{movement.rules.length - 3} more rules
            </p>
          )}
        </div>
          ...
          <div className="flex items-center gap-4 text-xs text-muted-foreground">
            {movement.adoptionCount > 0 && (
              <span className="flex items-center gap-1">
                ...
                {movement.adoptionCount.toLocaleString()} families
              </span>
            )}
            {movement.schoolCount > 0 && (
              <span className="flex items-center gap-1">
                <School className="w-3.5 h-3.5" />
                {movement.schoolCount} schools
              </span>
            )}
          </div>
...
export default function MovementsHubPage() {
  const [searchQuery, setSearchQuery] = useState("")
  const [tagFilter, setTagFilter] = useState<string | null>(null)
  const stats = useMemo(() => getMovementsStats(), [])

  // Get all unique tags
  const allTags = useMemo(() => {
    const tags = new Set<string>()
    MOVEMENTS_REGISTRY.forEach((s) => s.tags.forEach((t) => tags.add(t)))
        ...
        <div className="relative max-w-5xl mx-auto px-4 sm:px-8 py-20 sm:py-28">
          <AnimatedSection>
            ...
            <p className="text-base sm:text-lg text-white/60 max-w-2xl leading-relaxed mb-10">
              Browse {stats.total} community movements from the AAP, WHO, US Surgeon General, Common Sense Media,
              Fairplay, Thorn, and more. Adopt a movement for your child, and Phosra enforces it across every
              connected platform.
            </p>
            ...
            <div className="flex flex-wrap gap-8">
              {[
                ...
                { value: `${stats.active}`, label: "Active Movements" },
                { value: `${stats.total}`, label: "Total Movements" },
              ].map((stat) => (
                <div key={stat.label}>
      ...
      <section className="max-w-5xl mx-auto px-4 sm:px-8 pb-12">
        {filteredMovements.length === 0 ? (
          <div className="text-center py-16">
            <p className="text-muted-foreground">No movements match your search.</p>
          </div>
        ) : (
          <div className="grid sm:grid-cols-2 gap-6">
            {filteredMovements.map((movement) => (
              <MovementHubCard key={movement.id} movement={movement} />
            ))}
          </div>
        )}
      </section>
          ...
          <div className="grid md:grid-cols-3 gap-8">
            {[
              ...
              },
              {
                ...
                description:
                  "One click to adopt a community movement. Rules generate automatically based on the movement definition and your child's age.",
              },
              {
                number: "03",
                title: "Phosra enforces",
                description:
                  "Movements push to every connected platform. Families earn verified badges. Pledges become real, enforceable protection.",
              },
            ].map((step, i) => (
        ...
        <AnimatedSection>
          <div className="plaid-card text-center max-w-2xl mx-auto">
            <h2 className="text-xl sm:text-2xl font-display text-foreground mb-3">
              Want to define your own movement?
            </h2>
            <p className="text-muted-foreground text-sm mb-6 max-w-lg mx-auto">
              If you represent a child safety organization or school, you can define a community movement
