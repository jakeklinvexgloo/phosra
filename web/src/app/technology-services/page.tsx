...
import {
  TECH_SERVICE_ENTRIES,
  TECH_SERVICE_CATEGORIES,
  TECH_SERVICE_STATS,
  type TechServiceEntry,
} from "@/lib/platforms/adapters/to-technology-services"
...
export default function TechnologyServicesPage() {
  ...
  const featuredPlatforms = useMemo(
    () => TECH_SERVICE_ENTRIES.filter((p) => p.marquee),
    [],
  )

  /* Derived: all platforms grouped by category */
  const byCategory = useMemo(() => {
    const map: Record<string, TechServiceEntry[]> = {}
    for (const p of TECH_SERVICE_ENTRIES) {
  ...
  const searchResults = useMemo(() => {
    ...
    const filtered = TECH_SERVICE_ENTRIES.filter(
      (p) =>
        p.name.toLowerCase().includes(q) ||
        p.categoryLabel.toLowerCase().includes(q),
    )
    const groups: Record<string, TechServiceEntry[]> = {}
    ...
    <div>
      ...
      <div className="mb-8">
        ...
        <p className="text-muted-foreground text-lg max-w-2xl mb-4">
          Explore the {TECH_SERVICE_STATS.total} technology services Phosra enforces across{" "}
          {TECH_SERVICE_STATS.categoryCount} categories — streaming, social media, gaming,
          DNS filtering, devices, and more.
        </p>
        <div className="flex items-center gap-4 flex-wrap">
          <span className="inline-flex items-center gap-1.5 text-sm text-muted-foreground">
            <span className="w-2 h-2 rounded-full bg-emerald-500" />
            {TECH_SERVICE_STATS.integratedCount} integrated
          </span>
          <span className="inline-flex items-center gap-1.5 text-sm text-muted-foreground">
            <span className="w-2 h-2 rounded-full bg-gray-400" />
            {TECH_SERVICE_STATS.plannedCount}+ on roadmap
          </span>
        </div>
      </div>
      ...
      <div className="relative mb-5">
        ...
        <input
          type="text"
          placeholder="Search technology services by name or category"
          value={search}
      ...
      {isSearchActive ? (
        ...
        <div>
          <p className="text-sm text-muted-foreground mb-4">
            {searchResults?.total ?? 0} service{(searchResults?.total ?? 0) !== 1 ? "s" : ""} found
          </p>
          {searchResults && Object.keys(searchResults.groups).length > 0 ? (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {TECH_SERVICE_CATEGORIES.filter((c) => searchResults.groups[c.slug]).map((c) => (
                <CategoryCard
                  key={c.slug}
                  label={c.label}
        ...
        <div className="plaid-card !p-4">
          <div className="grid grid-cols-4 sm:grid-cols-6 lg:grid-cols-8 gap-1">
            {featuredPlatforms.map((platform) => {
              const meta = TECH_SERVICE_CATEGORIES.find((c) => c.slug === platform.category)
              return (
                <div
                  key={platform.id}
          ...
          <div className="mt-3 pt-3 border-t border-border text-center">
            <p className="text-xs text-muted-foreground">
              Showing {featuredPlatforms.length} featured services of{" "}
              {TECH_SERVICE_STATS.total} total.{" "}
              <button
                onClick={() => setActiveTab(TECH_SERVICE_CATEGORIES[0]?.slug as TabKey)}
        ...
        (() => {
          const meta = TECH_SERVICE_CATEGORIES.find((c) => c.slug === activeTab)
      ...
      <div className="mt-6 text-center text-sm text-muted-foreground">
        {TECH_SERVICE_STATS.total} technology services across {TECH_SERVICE_STATS.categoryCount} categories
      </div>
    </div>
  )
}
