...
import { AnimatedSection, WaveTexture, GradientMesh, MovementIcon } from "./shared"
import { MOVEMENTS_REGISTRY, getMovementsStats } from "@/lib/movements"
import type { MovementEntry } from "@/lib/movements"

function MovementCard({ movement, index }: { movement: MovementEntry; index: number }) {
  return (
    <div
      ...
      >
        <div className="relative h-full bg-white/[0.04] backdrop-blur-sm border border-white/[0.08] rounded-xl p-6 hover:bg-white/[0.07] hover:border-white/[0.14] transition-all duration-300">
          ...
          <div
            ...
            style={{ background: `linear-gradient(90deg, transparent, ${movement.accentColor}40, transparent)` }}
          />

          <div className="flex items-start justify-between mb-4">
            <div className="flex items-center gap-3">
              <MovementIcon movement={movement} size="md" />
              <div>
                <h3 className="text-base font-semibold text-white group-hover:text-brand-green transition-colors">
                  {movement.name}
                </h3>
                <p className="text-[11px] text-white/40">{movement.organization}</p>
              </div>
            </div>
...
export function Movements() {
  ...
  const visibleMovements = showAll ? MOVEMENTS_REGISTRY : MOVEMENTS_REGISTRY.slice(0, INITIAL_COUNT)
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-5 sm:px-6 lg:px-8">
        ...
        <AnimatedSection className="text-center mb-6">
          ...
          <p className="text-white/50 text-base sm:text-lg max-w-2xl mx-auto mb-10">
            ...
            movement and every connected platform enforces it automatically.
          </p>
        </AnimatedSection>
        ...
        <AnimatedSection delay={0.1} className="mb-12">
          <div className="flex flex-wrap justify-center gap-6 sm:gap-10">
            {[
              { value: `${stats.total}`, label: "Movements" },
        ...
        <AnimatedSection className="mb-12">
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5">
            {visibleMovements.map((movement, i) => (
              <MovementCard key={movement.id} movement={movement} index={i} />
            ))}
          </div>

          {!showAll && MOVEMENTS_REGISTRY.length > INITIAL_COUNT && (
            <div className="text-center mt-8">
              ...
              >
                Show All {MOVEMENTS_REGISTRY.length} Movements
            ...
            <div className="grid sm:grid-cols-3 gap-6 sm:gap-8">
              {[
                ...
                },
                {
                  ...
                  description: "One click to adopt a movement for your child. Rules are generated automatically from the movement definition.",
                },
                {
                  step: "03",
