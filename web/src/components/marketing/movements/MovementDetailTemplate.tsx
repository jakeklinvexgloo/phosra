...
function getRelatedLaws(movement: MovementEntry) {
  const movementCategories = new Set(movement.rules.map(r => r.category))
  return LAW_REGISTRY
    .filter(law => law.ruleCategories.some(c => movementCategories.has(c)))
    .sort((a, b) => {
      const aOverlap = a.ruleCategories.filter(c => movementCategories.has(c)).length
      const bOverlap = b.ruleCategories.filter(c => movementCategories.has(c)).length
      return bOverlap - aOverlap
    })
    .slice(0, 6)
}

function getRelatedControls(movement: MovementEntry) {
  const movementCategories = new Set(movement.rules.map(r => r.category))
  return PARENTAL_CONTROLS_REGISTRY
    .filter(control => control.capabilities.some(c => movementCategories.has(c.category)))
    .sort((a, b) => {
      const aOverlap = a.capabilities.filter(c => movementCategories.has(c.category)).length
      const bOverlap = b.capabilities.filter(c => movementCategories.has(c.category)).length
      return bOverlap - aOverlap
    })
    .slice(0, 6)
}
...
export function MovementDetailTemplate({ movement }: { movement: MovementEntry }) {
  ...
  const relatedLaws = getRelatedLaws(movement)
  const relatedControls = getRelatedControls(movement)
      ...
      {relatedLaws.length > 0 && (
        <section className="max-w-5xl mx-auto px-4 sm:px-8 py-12 sm:py-16">
          <AnimatedSection>
            ...
            <p className="text-sm text-muted-foreground mb-8">
              Laws and regulations that share rule categories with {movement.name}.
            </p>
          </AnimatedSection>
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {relatedLaws.map((law, i) => {
              const overlap = law.ruleCategories.filter(c =>
                movement.rules.some(r => r.category === c)
              ).length
              return (
      ...
      {relatedControls.length > 0 && (
        <section className="max-w-5xl mx-auto px-4 sm:px-8 py-12 sm:py-16">
          <AnimatedSection>
            ...
            <p className="text-sm text-muted-foreground mb-8">
              Parental control tools that support capabilities aligned with {movement.name}.
            </p>
          </AnimatedSection>
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {relatedControls.map((control, i) => {
              const overlap = control.capabilities.filter(c =>
                movement.rules.some(r => r.category === c.category)
              ).length
              return (
