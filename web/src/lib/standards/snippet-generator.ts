import type { StandardEntry, StandardRule } from "./types"
...
/* ── Hero snippet: adopt entire standard ───────────────────────── */

export function generateFullStandardSnippet(standard: StandardEntry): CodeExample {
  const code = `curl -X POST https://api.phosra.com/v1/enforcement/standards \\
  ...
  const response = JSON.stringify(
    {
      enforcement_id: `enf_${standard.slug.replace(/-/g, "_")}_01`,
      status: "active",
      standard: standard.name,
      rules_applied: standard.rules.length,
  ...
  return {
    title: `POST /v1/enforcement/standards`,
    language: "bash",
    code,
    response,
  }
...
export function generateRuleSnippet(standard: StandardEntry, rule: StandardRule): CodeExample {
